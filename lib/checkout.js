...
function calcChange(total, tendered) {
  ...
  if (!Number.isFinite(t) || t < total) throw new Error('預かり金額が合計より少ないです');
  return t - total;
}

function nextSlipNo(date, salesSameDay) {
  const ymd = String(date).replace(/-/g, '');
  const seq = String((salesSameDay || 0) + 1);
  if (seq.length > 3) throw new Error(`伝票No 桁数超過: ${seq}`);
  return `${ymd}-${seq.padStart(3, '0')}`;
}
