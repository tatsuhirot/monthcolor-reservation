...
async function launchHumanBrowser(statePath = null) {
  ...
  if (wsCdpUrl) {
    ...
    // ── フォールバック: Firefox + Webshare Residential Proxy ────────
    ...
    if (proxyServer) {
      ...
      console.log(`🦊 Firefox + Webshare Proxy で起動 (${proxyServer})`);
    } else {
      console.log('🦊 ローカルFirefox で起動（プロキシなし）');
    }
    ...
    if (proxyServer && proxyUser) {
      contextOptions.httpCredentials = { username: proxyUser, password: proxyPass };
    }
